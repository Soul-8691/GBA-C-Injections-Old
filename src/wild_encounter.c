...
bool8 TryGenerateWildMonLevelScaling(const struct WildPokemonInfo * info, u8 area, u8 flags)
{
    ...
    if (badgeCount == 0) level = (Random() % 4) + 9;
    else if (badgeCount == 1) level = (Random() % 6) + 15;
    else if (badgeCount == 2) level = (Random() % 6) + 23;
    else if (badgeCount == 3) level = (Random() % 6) + 29;
    else if (badgeCount == 4) level = (Random() % 6) + 33;
